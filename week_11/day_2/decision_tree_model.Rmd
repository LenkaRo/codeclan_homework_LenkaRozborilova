...
Looks like `age_status`, `sex` and `pclass` might be potentionally significant predictors

Let's have a closer look at some variables

```{r}
# seems like we lost a lot of observations by dropping NA and now, there is now more survivals in our data set than people who died (oposite to the original data set)
# that is one of the disadvantages of decision trees, they can't work with NA and dropping them might lead to biasing of the data

# looks like in proportion, children had hgher chance to survive than adults
titanic_set %>% 
  ggplot() +
...
* There is 147 passengers entering our model.
* The predicted result for a datapoint at the node (Survived or Not Survived in this example) is on the top line.
* The second line contains probability of a *not survived* result expressed as a decimal. So if a passanger was male, he had 0.46 chance of surviving. If a passanger was female, she had 0.91 chance to survive. 
* If the passanger was male and had 0 siblings aboard, he had 0.41 chance of surviving, while male with at least one sibling aboard had chance of surviving 0.53.
* Male passanger, who had at least one sibling aboard and embarked Titanic in Cherbourg, had 0.75 chance to survive. There was 5% of passangers aboard who would fit this categorization.
...
titanic_test_pred %>% 
  select("sex", "sib_sp", "embarked", "parch", "survived", "pred")
```
...
### the decision tree model is predicting 31 results correctly (21 TP + 10 TN) and 5 wrong (1 FN + 4 FP). It has sensitivity (= true positive rate) of 0.7143 and specificity (= true negative rate) of 0.9545.